-- 관리자용 시설 이용 분석.
--
-- 사무국이 이용 현황을 관찰하고 개선점을 찾기 위한 통계. 관리자만 조회할 수 있다.
--
-- 지표를 만들 때 조심한 것 세 가지:
--
-- 1) "이용 시간"은 예약한 시간이 아니라 '실제 점유 시간'으로 센다.
--    3시간 예약하고 30분 만에 취소했으면 30분이다. 예약 기준으로 세면 이용률이 부풀려진다.
--    실제 종료 = min(취소 시각, 예정 종료, 지금)
--
-- 2) 당일과 평균은 '같은 시각까지'로 잘라 비교한다.
--    당일은 아직 안 끝난 하루다. 완주한 하루의 평균과 그냥 비교하면 매일 오전마다
--    당일이 저조하게 보인다. 그래서 과거 날짜도 지금과 같은 시각까지만 잘라 평균을 낸다.
--
-- 3) status='active'는 "지금 이용 중"이 아니다.
--    끝난 예약도 active로 남는다(취소만 cancelled). 시간 조건 없이 세면 개설 이후 전부 잡힌다.
--
-- 자리비움 자동취소는 별도 컬럼 없이 구분한다. cancel_stale_away()가 status만 바꾸고
-- away_since를 남기므로, cancelled + away_since 있음 = 자동취소로 본다(휴리스틱).
