-- 시설 이용 분석에 '이용 인원' 지표를 더한다.
--
-- 좌석 점유율·예약 건수와 별개로, "며칠 동안 몇 명이 왔나"를 본다. 한 사람이 하루에
-- 여러 번 예약해도 그날은 1명으로 센다(고유 인원). '왔다'의 기준은 그날 예약이 한 번이라도
-- 있었는지다(예약하려면 센터 와이파이가 필요하므로 온 것으로 본다).
--
-- 추가되는 값(admin_stats 반환 jsonb):
--   current_users : 지금 이 순간 좌석을 점유 중인 인원(날짜 범위와 무관, 항상 '지금').
--   users_total   : 선택 기간에 한 번이라도 이용한 고유 인원(오늘 포함).
--   users_avg     : 하루 평균 이용 인원(완료된 날 기준, 오늘 제외 — 다른 평균과 동일).
--   weekday       : 요일별 하루 평균 이용 인원. dow=1(월)~7(일), 이용이 있었던 날만 평균.
--
-- 나머지(도넛 2개·시간대별·좌석별)는 0029와 동일하다. create or replace로 본문만 바꾼다.
