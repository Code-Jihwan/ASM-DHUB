-- 취소를 '예약 취소'와 '좌석 반납'으로 구분해 통계·좌석 이력에 반영한다.
--
-- 10분 미만 쓰고 그만두면 '예약 취소'(안 쓴 셈), 10분 이상 쓰고 그만두면 '좌석 반납'
-- (정상 이용). 경계는 policy.cancel_grace()(=쿨다운 유예선)와 같다. 화면 쪽 isSeatReturn과 짝.
--
-- 두 함수를 고친다.
--   admin_stats        : '종료 유형'을 정상 종료 / 좌석 반납 / 예약 취소 / 자리비움 자동취소로 나눔
--   admin_seat_history : 좌석 이력에 cancelled_at을 실어, 화면이 반납/취소를 가릴 수 있게 함
...
begin
  ...
  outcome as (
    select
      ...
      -- ord의 분기 순서는 위 kind와 반드시 같아야 한다(다르면 한 행의 kind와 ord가 어긋난다).
      case
...
------------------------------------------------------------------ 좌석 이력에 취소 시각 추가
-- 반환 컬럼이 늘어 create or replace로는 안 되므로 drop 후 재생성한다(0009와 같은 방식).
...
begin
  ...
  return query
    select r.id, r.period, r.extended, r.status, r.away_since, r.cancelled_at,
