...
좌석 예약·자리비움·연장 같은 기본 흐름부터, 회의실 현황 조회(전체 공개)·예약 알림(웹 푸시),
관리자 운영 도구(명단·신고·공지·이용 분석)까지 갖췄다.
...
<p align="center">
  <img src="docs/release-note-v1.3.0.png" alt="@@자리요 v1.3.0 릴리스 노트" width="760">
</p>

> 원본 HTML: [`docs/release-note-v1.3.0.html`](docs/release-note-v1.3.0.html) — 브라우저에서 열어 인쇄(PDF 저장)할 수 있습니다.
> 이전 버전: [v1.2.0](docs/release-note-v1.2.0.html) · [v1.1.0](docs/release-note-v1.1.0.html) · [v1.0.0](docs/release-note-v1.0.0.html)
...
- **회의실 현황**(`/rooms`) — 18F 회의실 7개(M1~M3 / A1~A4)의 하루 일정을 배치도·타임라인으로 조회. **로그인한 모든 사용자에게 공개(조회 전용)** — 예약 없는 빈 회의실을 바로 확인. 데이터는 자동 동기화로 갱신(관리자 수동 업로드도 가능)되며 서버에 저장해 공유된다
...
### 7. 회의실 현황 조회 (전체 공개, 조회 전용)

18F 회의실 7개(M1~M3 / A1~A4)의 하루 일정을 배치도·타임라인으로 조회한다(조회 전용 — 예약 생성/수정은 없음).
**로그인한 모든 사용자**가 볼 수 있다.

- `0036_meeting_room_snapshot.sql` + `0037_meeting_room_snapshot_public.sql` 실행. 파싱 결과(JSONB)를 **단일 스냅샷 행**에 저장해 공유한다.
- 데이터는 8번 자동 동기화로 갱신(관리자페이지의 "회의실 예약 현황" 칸에서 수동 업로드도 가능). 저장/삭제는 관리자만(security-definer 함수), **읽기는 로그인한 모든 사용자(0037로 개방)**.
- 공개 범위: 예약자 이름·예약명을 **마스킹 없이 그대로 노출**(운영자 방침). 개인정보를 가리려면 서버(RPC/뷰)에서 마스킹이 필요하다.
...
### 8. 회의실 현황 자동 동기화 (기본 운영 방식)

**회의실 현황은 이 자동 동기화로 갱신한다.** 사내 상시가동 윈도우 PC의 작업 스케줄러가 하루 5회(09/12/15/18/21시)
예약 시스템 엑셀을 받아 자리요에 자동 반영한다. (관리자 **수동 업로드** UI도 그대로 있어 **폴백**으로 쓸 수 있다.)
...
- **회의실 현황 자동화** — 사내 상시가동 윈도우 PC의 작업 스케줄러가 하루 5회 자동 동기화한다(8번). 다만 **그 PC에
  의존**한다(꺼짐·로그오프·절전 시 중단). 소스 시스템이 API/정기 이메일 export를 제공하면 PC 의존 없이 서버 크론으로
  옮길 수 있다. (관리자 수동 업로드도 폴백으로 유지.)
- **회의실 현황 개인정보** — 현재 로그인한 모든 사용자에게 전체 공개하며 예약자 이름·예약명을 그대로 노출한다(운영자 방침). 추후 정책이 바뀌면 서버(RPC/뷰)에서 마스킹하는 공개용 뷰로 좁힐 수 있다.
